...
class DeepSeekProvider:
    ...
    def _recommendation_validation_intents(
        ...
        def registered_metrics(
            ...
            for field in metric_fields:
                ...
                if (
                    ...
                    and definition.semantic not in allowed_semantics
                ):
                    raise PlanCompilationError(
                ...
                if (
                    definition is None
                    or definition.semantic in seen_semantics
                ):
                    continue
                metrics.append(
                    IntentMetric(
            ...
            for date_field in fields:
                ...
                for dimension_field in fields:
                    ...
                    if len(metric_fields) > 3 or any(
                        not (
                            is_metric_field(field) or is_numeric_code(field)
                        )
                        for field in metric_fields
                    ):
                        continue
        ...
        categorical_fields = [
            field
            for field in dimension_candidates
            if not is_numeric_code(field)
        ]
